...
struct vs_input_t
{
	...
	int    originalVertexIndex : ORIGINALINDEX;
};
...
cbuffer PerFrameConstants : register(b4)
{
	...
	int   c_debugInt; 
	float c_debugFloat;
	float skeletonWeightThreshold; //bones with weights below this value are ignored
	float4x4 boneMatrices[65];
};
...
v2p_t VertexMain(vs_input_t input)
{
    ...
    for(int i = 0; i < 4; i++)
    {
		...
		if (weight > 0.0f)
		{
			skinnedPosition	+= weight * mul(boneMatrix, float4(input.modelPosition, 1.f));
			skinnedTangent	+= weight * mul(boneMatrix, float4(input.modelTangent, 0.f));
			skinnedBitangent+= weight * mul(boneMatrix, float4(input.modelBitangent, 0.f));
			skinnedNormal	+= weight * mul(boneMatrix, float4(input.modelNormal, 0.f));
		}
    }
	// Finish Skinning and hand skinned positions to the normal Model to World to Clip pipeline

	float4 modelPosition	= skinnedPosition;
	...
	float4 modelTangent		= normalize(skinnedTangent);
	float4 modelBitangent	= normalize(skinnedBitangent);
	float4 modelNormal		= normalize(skinnedNormal);
